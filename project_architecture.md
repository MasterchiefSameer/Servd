...
Based on a deep scan of your codebase (both `frontend` and `backend` directories), here is the comprehensive architectural overview of your application, detailing every library, language, route structure, and the exact flow of data!
...
Based on your `app/(main)` and `app/(auth)` folders, the router structure provides:
...
- `/recipes`: The master list of all recipes they've requested or saved.
...
Your backend acts as the single source of truth for saving user-generated content natively, independent of Clerk.

### Key Libraries
*   **Strapi (v5.40)**: The headless Node.js CMS that instantly spins up REST API endpoints. You run it using custom Node DNS flags to bypass IPv6 connection timeouts (an incredibly smart fix for local Windows development!).
*   **pg (PostgreSQL client)**: Drives the direct connection engine connecting Strapi to your serverless **Neon DB** in the cloud.

### Data Models (`backend/src/api/`)
Your Strapi schema consists of four primary internal entities logic:
