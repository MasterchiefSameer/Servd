//Main recipe page, (My Recipes) "View Full Recipe"
...
import { AlertCircle, ArrowLeft, Bookmark, BookmarkCheck, Loader2 } from 'lucide-react';
...
function RecipeContent() {
  ...
  // console.log(recipeData) // it show the dummy data in browser console
  // Save to collection
  const {
    loading: saving,
    data: saveData,
  ...
  useEffect(() => {
    if (removeData?.success) {
      setIsSaved(false); //set the isSaved to false
  ...
  if (!recipeName) { // if no recipe is provided.
    return (
  ...
  // Loading state
  //  if (loadingRecip === null || loadingRecipe == true)
        ...
        <div className="grid lg:grid-cols-3 gap-6">
          ...
          <div className="lg:col-span-1 space-y-6">
            ...
            <div className="bg-white p-6 border-2 border-stone-200 lg:sticky lg:top-24">
              ...
              {/* Group by category */}
              {Object.entries(
                recipe.ingredients.reduce((acc, ing) => { // accumulator - acc, {} empty by default
                  const cat = ing.category || "Other";
                  if (!acc[cat]) acc[cat] = []; // if the category is not found, create it
                  acc[cat].push(ing); // push the ingredient to the category -- GRAIN, VEGETABLES, SPICES
          ...
          <div className="lg:col-span-2 space-y-6">
            ...
            {recipe.instructions.map((step, index) => (
              <div key={step.step}
                className={`relative pl-12 pb-8 ${index !== recipe.instructions.length - 1 ? "border-b-2 border-stone-300 ml-5" : "ml-5"
                  }`}>
                {/* Step Number */}
  ...
  return (
    <Suspense
      fallback={
        ...
        <div className="min-h-screen bg-stone-50 pt-24 pb-16 px-4">
          <div className="container mx-auto max-w-4xl text-center py-20">
            <Loader2 className="w-16 h-16 text-orange-600 animate-spin mx-auto mb-6"></Loader2>
            <p className="text-stone-600 text-lg">Loading recipe...</p>
          </div>
        </div>
      }
    >
