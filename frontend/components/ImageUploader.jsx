...
export default function ImageUploader({ onImageSelect, loading }) {
  ...
  if (preview) {
    return (
      <div className="relative w-full aspect-video bg-stone-100 rounded-2xl overflow-hidden border-2 
      border-stone-200">
        <Image
          src={preview}
          alt="Pantry preview"
        ...
        {loading && (
          <div className="absolute inset-0 bg-black/40 flex items-center justify-center">
            ...
            <RingLoader color="white"></RingLoader>
          </div>
        )}
      </div>
    );
    ...
    <>
      <div
        ...
        className={`relative w-full aspect-square border-2 border-dashed rounded-2xl transition-all cursor-pointer ${
          isDragActive
            ? "border-orange-600 bg-orange-50 scale-[1.02]"  //case when cursor is not on area
